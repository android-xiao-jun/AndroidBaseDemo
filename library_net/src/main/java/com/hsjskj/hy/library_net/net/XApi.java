...
import com.hsjskj.hy.library_net.net.converter.FastJsonConverterFactory;
import com.hsjskj.hy.library_net.net.cookie.CookieJarImpl;
import com.hsjskj.hy.library_net.net.cookie.PersistentCookieStore;
import com.hsjskj.hy.library_net.net.progress.ProgressHelper;
...
public class XApi {
    public static final long connectTimeoutMills = 15 * 1000l;//连接时间增加
            ...
            public Publisher<T> apply(Flowable<T> upstream) {
                return upstream.flatMap(new Function<T, Publisher<T>>() {
                    ...
                    public Publisher<T> apply(T model) throws Exception {
                        if (model == null) {
                            return Flowable.error(new NetError("", "", NetError.OtherError));
                        } else if (model.isRepeatLogin()) {
                            return Flowable.error(new NetError(model.getErrorMsg(), model.getCode(), NetError.RepeatLogin));
                        } else if (model.isNull()) {
                            return Flowable.error(new NetError(model.getErrorMsg(), model.getCode(), NetError.NoDataError));
                        } else if (model.isAuthError()) {
                            return Flowable.error(new NetError(model.getErrorMsg(), model.getCode(), NetError.AuthError));
                        } else if (model.isBizError()) {
                            return Flowable.error(new NetError(model.getErrorMsg(), model.getCode(), NetError.BusinessError));
                        } else {
...
//                .addConverterFactory(GsonConverterFactory.create());
    ...
    private OkHttpClient getClient(String baseUrl, NetProvider provider) {
        ...
        CookieJarImpl cookieJarImpl = new CookieJarImpl(new PersistentCookieStore(provider.initContext()));
